...
const Register: React.FC = () => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const userData = {login: username, password: password}; // Création de l'objet contenant les données
            const response = await axios.post('http://127.0.0.1:8000/api/account/register', userData); // Envoi des données au serveur
            console.log('Compte créé avec succès:', response.data);
            navigate('/login'); // Redirection vers le formulaire d'autres informations
        } catch (error) {
